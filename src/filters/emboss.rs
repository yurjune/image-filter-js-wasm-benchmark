...
// Emboss kernel (3x3)
const KERNEL: [i32; 9] = [-2, -1, 0, -1, 1, 1, 0, 1, 2];
const OFFSET: i32 = 128;

#[wasm_bindgen]
pub fn emboss_wasm(pixels: &mut [u8], width: u32, height: u32) {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            pixels[dst_idx] = (r_sum + OFFSET).clamp(0, 255) as u8;
            pixels[dst_idx + 1] = (g_sum + OFFSET).clamp(0, 255) as u8;
            pixels[dst_idx + 2] = (b_sum + OFFSET).clamp(0, 255) as u8;
